// Crea las 7 filas de bloqueo de conferencia en Citas (nueva) de Adler.
// No crea el contacto ficticio (ya existe). No toca el workspace de Laura.
...
//
//   node -r dotenv/config scripts/one-shots/crear-bloqueos-conferencias.js --confirmar

const { notionFetch } = require('../../src/utils/notion-client');

const CONTACTOS_PRUEBAS = '9f335308-da0e-4672-9744-c1dabcfb22aa';
const CITAS_PRUEBAS = 'df93bc94-26ee-42fc-92d7-a0ed3a8e1f68';
const BLOQUEO_ID = '3c990fe2-7345-8121-92a6-f9e09a540d2e';
const PRODUCCION_CONTACTOS_PREFIX = '3b162dda';

process.env.NOTION_CONTACTOS_DATA_SOURCE_ID = CONTACTOS_PRUEBAS;
process.env.NOTION_CITAS_DATA_SOURCE_ID = CITAS_PRUEBAS;
...
async function buscarSponsorPorEmpresa(item) {
  ...
  for (const nombre of nombres) {
    const filas = await queryPaginado(CONTACTOS_PRUEBAS, {
      and: [
  ...
  const todos = await queryPaginado(CONTACTOS_PRUEBAS, {
    property: 'Categoria', select: { equals: 'Sponsor' },
  });
...
async function buscarBloqueoExistente({ sponsorId, inicio }) {
  const filas = await queryPaginado(CITAS_PRUEBAS, {
    and: [
...
async function crearFila({ sponsor, item, inicio, fin }) {
  ...
  return notionFetch('/pages', {
    method: 'POST',
    body: JSON.stringify({
      parent: { type: 'data_source_id', data_source_id: CITAS_PRUEBAS },
      properties: {
...
async function main() {
  if (!process.argv.includes('--confirmar')) {
    console.error('Falta --confirmar. Este script escribe en Citas (nueva) de Adler.');
    process.exit(1);
  }
  if (!process.env.NOTION_API_KEY) {
    throw new Error('Falta NOTION_API_KEY');
  }
  if (String(process.env.NOTION_CONTACTOS_DATA_SOURCE_ID || '').startsWith(PRODUCCION_CONTACTOS_PREFIX)) {
    throw new Error('Abortado: el data source de Contactos parece el de producción de Laura.');
  }

  const dsContactos = await notionFetch(`/data_sources/${CONTACTOS_PRUEBAS}`);
  const tituloContactos = (dsContactos.title || []).map((t) => t.plain_text).join('');
  console.log(`Contactos: ${tituloContactos} (${CONTACTOS_PRUEBAS})`);
  if (/laura/i.test(tituloContactos) && !/nueva/i.test(tituloContactos)) {
    throw new Error(`Abortado por título de Contactos: ${tituloContactos}`);
  }
  ...
  console.log(`✅ Live: Blip ocupado a ${blip.inicio}; mesas reales en ese bloque = ${mesas} (el bloqueo no suma).`);
}
