...
| `sugerir_matches_para_sponsor` | Escritura acotada | Matchmaking para un sponsor específico. `escribirEnNotion` default `false` (dry-run) — con `true`, crea una fila `Sugerido` en `Citas` por candidato. Capa 1: Giro/Industria (Marca de moda, Retailer, Manufactura; sin excepción para VIP/Speaker), `Quiere Citas 1a1` excluye solo `'No'`, Tamaño de Negocio / Madurez Exa (**Presencial VIP y Speaker saltan tamaño**, vía `ticketTipo`, 1-sep). `Tamaño de Negocio` se lee como texto libre o select transitorio y se clasifica por prefijo. **No** filtra por `Etapa de Negocio` (28-ago). El objeto `sponsor` del return incluye `calendarioGoogleId` desde el 13-ago |
...
**Tamaño libre + Speaker (1-sep):** `Tamaño de Negocio` es `rich_text`; el parser conserva lectura dual de `rich_text`/`select` durante la transición y clasifica por prefijo Grande/Mediana/Pequeña/Micro sin distinguir acentos. Otros textos son respuestas legacy de Etapa y caen al fallback de `Madurez Negocio (Exa)`. El boleto `Speaker` entra al pool, conserva el filtro de Giro y salta el filtro de tamaño. **Ranking (3-sep):** el canal ya no suma puntos fijos; `Speaker` y `Presencial VIP` multiplican la afinidad por 1.4, `Presencial` por 1.15, `Virtual` por 1.0. Oro molido (+1000) queda fuera del multiplicador. Pendiente de validar con Laura/Liz.
...
- **Campañas de matchmaking**:
  ...
  - El envío real exige `PLATICA_TEMPLATE_OFERTA_INICIAL` (nombre en Meta: `agendar_cita_inicial`, aprobada 28-ago). **Contrato: 2 variables.** `{{1}}` = **primer nombre** del asistente, capitalizado (`primerNombreParaSaludo`: Ticketópolis vuelca "ANA MARIA PEREZ LOPEZ" en `Nombre` y el saludo salía gritado y con apellidos; se toma solo el primer token, sin adivinar nombres compuestos, y si queda vacío se usa `Asistente`); `{{2}}` = hasta 4 sponsors con su solución, en **un solo renglón** con una viñeta por sponsor (`• *Empresa* (solución)`, separador ` • `) y el nombre en negritas. La solución que se muestra es la **intersección** de `Solucion` del sponsor con `Soluciones Buscadas` del asistente, con tope de 2 por sponsor y sin el comodín `Otro` — así el asistente lee por qué le proponen a ese sponsor y el cuerpo no se pasa del tope de 1024 caracteres de Meta (con el catálogo completo, un caso de 4 sponsors medía 1035 y el envío se rechazaba). Si el asistente no declaró soluciones buscadas (registro legacy) se muestran las del sponsor; si el sponsor solo tiene `Otro`, va únicamente `*Empresa*`. Como último resguardo, `textoSugerencias` baja a 1 solución por sponsor cuando `{{2}}` pasa de 400 caracteres. WhatsApp rechaza el envío si el valor de una variable trae saltos de línea, tabs o más de 4 espacios seguidos, así que `payloadPara` sanea nombre y lista antes de mandarlos. Por eso las viñetas van en línea y no en renglones: el `\r` sí pasa el filtro de Meta, pero es retorno de carro y se come lo que sigue (probado el 2-sep con dos envíos reales a un número propio). Un renglón por sponsor exigiría meter la viñeta en el cuerpo fijo (`• {{2}}` / `• {{3}}` / …) y, como Meta no acepta parámetros vacíos, una plantilla por cantidad de sponsors.
