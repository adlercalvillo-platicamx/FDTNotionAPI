...
| `consultar_disponibilidad_cita` | Lectura | Recibe el `sponsorPageId` y devuelve como máximo 3 bloques libres en `opciones_para_ofrecer`. Sin `fecha`: casillas Día 1 Mañana / Día 1 Tarde / Día 2 (relleno si una casilla está vacía). Con `fecha`, solo ese día. Omite bloques que ya superaron el margen temporal de modificación. Si `hay_mas`, otra llamada con `excluirInicios`. Foto; `reservar_cita` / `modificar_cita` revalidan dentro del mutex. |
