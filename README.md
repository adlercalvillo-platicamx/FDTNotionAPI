...
| POST | `/citas/programar-recordatorio-15min` | **Nueva (3 sep).** Programa en Plática la plantilla `notificacion_cita_15min_antes` 15 min antes del `inicio`, solo al asistente (`{{1}}` primer nombre, `{{2}}` empresa del sponsor). `X-API-Key`. No escribe Notion. El camino feliz lo dispara **solo** `POST /citas/reservar` (fire-and-forget en el controller; si Plática falla, el 201 de agendar no cambia). Esta ruta queda para reintento manual. Si falta env, WhatsApp o el horario ya pasó → `{ omitido: true, motivo }`. Fallo de Plática en esta ruta → 502. Modificar/cancelar **no** anulan el programado. |
| POST | `/citas/reservar` | Reserva una cita 1a1 (mutex + Notion como árbitro). Asigna mesa 1–11 por orden de confirmación en el bloque; genera el título `Cita — Empresa asistente - Empresa sponsor`; envía correo + `.ics` al sponsor (con datos del asistente) y al asistente (solo nombre de empresa del sponsor). El `.ics` lleva `LOCATION` = Club France (dirección completa + `GEO`); mesa y horario van en el cuerpo y en `DESCRIPTION`. Si el correo falla tras 3 SMTP inmediatos, la cita **sí queda creada** con Estatus `Confirmada sin notificar`. `sponsor_calendario_id` en el body es legado e ignorado. Rechaza `ASISTENTE_YA_OCUPADO` si esa persona ya tiene cita en el mismo bloque. Si Notion no encuentra el contacto de la relación, responde `404 SPONSOR_NO_ENCONTRADO` / `ASISTENTE_NO_ENCONTRADO` nombrando el campo (antes era un `500` opaco; 2-sep). |
| POST | `/citas/modificar-cita` | **Nueva (27 de agosto).** Mueve una cita real a otro bloque. Identificación por `telefono` (el servidor valida que la cita sea de esa persona) o por `citaId` directo (Laura/Liz). Valida el horario nuevo con el mismo criterio que `reservar` (grilla, 11 mesas, sponsor ocupado, bloqueos de conferencia) **antes** de tocar Notion; reasigna mesa y manda el `.ics` actualizado (mismo UID, `SEQUENCE` mayor, `LOCATION` Club France). El correo de cambio es texto propio (horario nuevo/anterior + mesa + sede); el sponsor recibe datos del asistente y el asistente solo la empresa del sponsor. Si el correo falla, el horario nuevo **se conserva** y la cita queda `Confirmada sin notificar`. |
...
**Tamaño libre + Speaker (1–4 sep):** `Tamaño de Negocio` es `rich_text`; el parser conserva lectura dual de `rich_text`/`select` durante la transición y clasifica por prefijo Grande/Mediana/Pequeña/Micro sin distinguir acentos. Desde 4-sep, un tamaño nuevo entra solo si el sponsor lo eligió en `Etapa Cliente Buscada`. Pequeña/Micro no reciben bono de tamaño; si Exa los marcó Consolidado/PyME, ese fallback **sí suma** en Capa 2. Otros textos son respuestas legacy de Etapa y siguen cayendo al fallback de `Madurez Negocio (Exa)` (Consolidado/PyME), sin cruzarse con la selección del sponsor. `Speaker` y `Presencial VIP` mantienen el bypass de tamaño decidido por Adler; Giro no tiene bypass. **Ranking (3-sep):** `Speaker`/VIP ×1.4, Presencial ×1.15, Virtual ×1.0; oro molido (+1000) no se multiplica. Pendiente de validar con Laura/Liz.
...
- **Campañas de matchmaking**:
  ...
  - El envío real exige `PLATICA_TEMPLATE_OFERTA_INICIAL` (nombre en Meta: `agendar_cita_inicial`, aprobada 28-ago). **Contrato: 2 variables.** `{{1}}` = **primer nombre** del asistente, capitalizado (`primerNombreParaSaludo`: Ticketópolis vuelca "ANA MARIA PEREZ LOPEZ" en `Nombre` y el saludo salía gritado y con apellidos; se toma solo el primer token, sin adivinar nombres compuestos, y si queda vacío se usa `Asistente`); `{{2}}` = hasta 4 sponsors en **un solo renglón**, numerados y separados por ` | ` (`1) *Marco Trujillo* de *Platica.mx* (solución)`). El representante es nombre + apellido paterno (`nombreRepresentanteParaOferta`: 2 tokens intactos; 3+ = primero + penúltimo). Negrita en persona y empresa. La solución es la **intersección** de `Solucion` del sponsor con `Soluciones Buscadas` del asistente, tope 2, sin `Otro`. Sin intersección (legacy) se muestran las del sponsor; si el sponsor solo tiene `Otro` o no hay representante, cae a `*Empresa*`. El cuerpo armado no puede pasar **1024 caracteres** (caso real 28-ago: catálogo completo = 1035). `textoSugerencias` calcula el techo de `{{2}}` como 1024 − cuerpo fijo − `{{1}}` − colchón y recorta 2 soluciones → 1 → nombres → soltar el último sponsor. WhatsApp rechaza el envío si el valor de una variable trae saltos de línea, tabs o más de 4 espacios seguidos, así que `payloadPara` sanea nombre y lista. Un renglón por sponsor exigiría meter la viñeta en el cuerpo fijo y una plantilla por cantidad: el `\r` se comió sponsors 2–4 (2-sep).
  ...
  - La oferta inicial solo se envía a un contacto sin `Última Campaña Enviada`. B, C1 y C2 siguen fuera del flujo activo; la única reactivación automática es el follow-up 72h descrito abajo.
  ...
  - **Follow-up 72h:** `PLATICA_TEMPLATE_FOLLOWUP_72H=followup_72hrs`, `FOLLOWUP_72H_MODO_SIMULACION=true`, `FOLLOWUP_72H_ENVIO_REAL_HABILITADO=false`. `PLATICA_WORKSPACE_ID`, `PLATICA_CHANNEL_ID=wb-1167456423128610` (Adler, 4-sep) y `PLATICA_WEBHOOK_SECRET` fijan y autentican el webhook `message.created`. `PLATICA_RESPONDER_AGENT_ID=c1IYnFsr0Jzfqq4NeLAs` viaja en cada plantilla para que el Agente 2 reciba las respuestas cuando Plática desacople canal y agente. Cualquier `incoming` posterior a `Fecha Última Campaña` marca `Respondió Oferta Inicial`; el cron vuelve a leer todas las conversaciones del teléfono/canal para cubrir webhooks perdidos. Vencimiento = 72 horas naturales; si cae fuera de lun–vie 09:00–18:00 `America/Mexico_City`, espera la siguiente ventana. Omite `Confirmada`, `Confirmada sin notificar`, `Pendiente Calendar` y `Completada`. Estado en Contactos: `Estado Follow-up 72h` (`En curso`/`Enviado`/`Falló`) + `Fecha Follow-up 72h`; un `En curso` vencido se reconcilia contra el mensaje saliente antes de reintentar.
