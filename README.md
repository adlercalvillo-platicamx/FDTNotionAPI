...
| `consultar_sugeridas_para_asistente` | Lectura | Campo **`sugeridas`**: solo filas `Aprobado`. **`sugeridas_para_ofrecer` hasta 4** mezcla primero canceladas reagendables (`para_reagendar=true`) y luego `Aprobado`; el resto va en `sponsors_para_agendar`. Aparte, `citasConfirmadas` (tope 3) y `citasCanceladas` (tope 3). Un sponsor con cita real no se vuelve a ofrecer. Una cancelada que ya originó otra cita no vuelve a ofrecerse. Identificador: `whatsapp`. |
