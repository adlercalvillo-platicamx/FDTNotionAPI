...
├── sugeridas-empresas.manual-test.js  # Empresas hidratadas + opciones_adicionales (14-sep)
...
| POST | `/reserva-publica/identificar` | Público con rate limit. `{ email, contactoId? }`; busca coincidencia exacta en Contactos. Si el correo corresponde a varios asistentes activos, responde 409 `EMAIL_AMBIGUO` con las opciones mínimas (id, nombre, empresa, boleto); la UI pide elegir y repite con `contactoId`, que se revalida contra ese mismo correo antes de emitir el token. Luego exige boleto `Presencial` / `Presencial VIP` / `Virtual` / `Speaker` **y** giro de los 3 elegibles. Expo → 403. Giro fuera → 403 `GIRO_NO_ELEGIBLE` (incluye el giro registrado). Devuelve token temporal, perfil mínimo, citas confirmadas y canceladas reagendables. `Quiere Citas 1a1=No` no bloquea. Tamaño/área/soluciones no filtran. |
...
| GET | `/reserva-publica/disponibilidad?sponsor=...&fecha=YYYY-MM-DD&exceptCitaId=` | Bearer temporal. Reusa la disponibilidad real incluyendo ocupación del asistente. `exceptCitaId` (modificar) omite esa fila para no marcar ocupado el bloque actual. |
...
| POST | `/reserva-publica/citas/:citaId/modificar` | Bearer temporal. `{ inicio }`. Misma `modificarCita` del Agente 2; la cita debe ser del contacto del token (`CITA_NO_PERTENECE`). |
| POST | `/reserva-publica/citas/:citaId/cancelar` | Bearer temporal. Misma `cancelarCita`; ownership del token. Tras cancelar se puede reagendar con `reservar` (fila nueva). |
...
| POST | `/citas/reservar` | Reserva una cita 1a1 (mutex + Notion como árbitro). El campo opcional `sponsor_empresa_confirmada` funciona como guarda conversacional: si no corresponde al `sponsor_notion_id`, rechaza `SPONSOR_EMPRESA_NO_COINCIDE` antes de escribir. La tool de Plática lo exige con la empresa canónica de `sponsor_solicitado`. Rechaza `BOLETO_EXPO_NO_PERMITE_CITAS` antes de escribir si el asistente tiene boleto `Expo`: solo incluye piso de exhibición. Rechaza `HORARIO_EN_PASADO` si el bloque ya superó `CITAS_MARGEN_MODIFICACION_MINUTOS` (mismo umbral que disponibilidad). Un reintento con el mismo `request_id` de una Confirmada no se rechaza por eso. Asigna la primera mesa física libre entre 1–11 en el bloque; si una cita se canceló, reutiliza ese hueco sin duplicar una mesa todavía ocupada. Para reagendar una cancelada recibe `cita_origen_cancelada_id`: valida `Cancelada` + mismo par, crea una fila distinta enlazada y rechaza reutilizar ese origen otra vez. Genera el título `Cita — Empresa asistente - Empresa sponsor`; envía correo + `.ics` al sponsor (copy de negocios con fecha/hora/mesa/sede y datos del asistente) y al asistente (sin teléfono/correo del sponsor). Para `Ticket / Tipo Asistencia = Virtual`, el cuerpo y `DESCRIPTION` del asistente omiten mesa, sede e indicación de llegada y avisan que el Meet llega ~15 min antes; el del sponsor conserva mesa y sede y suma `Modalidad: Google Meet` con la nota de tomarla desde su mesa; los demás boletos conservan mesa y Club France. El `.ics` mantiene `LOCATION` = Club France (dirección completa + `GEO`). Si el correo falla tras 3 SMTP inmediatos, la cita **sí queda creada** con Estatus `Confirmada sin notificar`. `sponsor_calendario_id` en el body es legado e ignorado. Rechaza `ASISTENTE_YA_OCUPADO` si esa persona ya tiene cita en el mismo bloque. |
...
| POST | `/citas/cancelar-cita` | **Nueva (27 de agosto).** Mismos dos caminos de identificación. Pasa el `Estatus` a `Cancelada` (con eso el bloque queda libre) y manda el `.ics` de baja (`METHOD:CANCEL`, `LOCATION` Club France). El cuerpo del sponsor nombra el par de empresas, fecha y horario cancelado (sin mesa ni sede) y los datos del asistente; el del asistente nombra a la encargada + empresa. Si el correo falla, la cita sigue cancelada y queda marcada para reintento. Llamarla dos veces es idempotente. |
| GET | `/citas/sugeridas?whatsapp=...&folio=...&sponsor_empresa=...` | Solo lectura salvo la hidratación de Plática al identificar por folio. Identifica primero por teléfono y, si no coincide, acepta `Folio Reservacion` o `Folio Boleto` (también si una celda trae varios separados por coma). `sponsor_empresa` resuelve el nombre aproximado del QR y devuelve `sponsor_solicitado` con elegibilidad del par: boleto + giro + tamaño; en este camino directo no bloquean `Quiere Citas=No`, área ni soluciones. Sin sponsor nombrado, `motivo_sin_opciones` distingue `GIRO_NO_ELEGIBLE` de `OPCIONES_AGOTADAS`. También devuelve `fase_evento`, copys contextuales y **todas** las citas confirmadas con día/hora. Sin teléfono → `CONTACTO_NO_RESUELTO` + `requiere_folio`; folio inválido/ambiguo usa error explícito. |
...
| POST | `/citas/reintentar-notificaciones-pendientes` | **Nueva (18 de agosto; granular 20-sep).** A demanda (no cron): reenvía correo/.ics de las citas `Confirmada sin notificar` y de las `Cancelada` cuyo aviso de baja nunca salió. `Notas Envio Email` guarda `[EMAIL_PENDIENTES:sponsor]`, `[EMAIL_PENDIENTES:asistente]` o ambos; el barrido reenvía **solo** esos lados. Filas legacy sin marcador reintentan ambos por compatibilidad. Omite bloqueos de conferencia. Sin tope. |
| POST | `/citas/:id/reenviar-notificacion` | Reenvía únicamente los destinatarios pendientes de **una** cita, con la misma semántica granular del barrido. La ruta estática de arriba va **antes** de `/:id` a propósito. |
...
| POST | `/matchmaking/enviar-recordatorio-evento` | **Deshabilitado 22-sep (Adler).** `X-API-Key`. Responde 200 `{ disparado: false, motivo: 'RECORDATORIO_EVENTO_DESHABILITADO' }` sin Notion ni Plática, aunque el cron de Coolify siga pegando y las banderas de campaña estén en envío real. No hay tool MCP ni env para reactivarlo. |
...
| POST | `/notion/respaldar` | **Nueva (23 sep; vistas 23 sep noche).** Cron diario ~03:00 CDMX. Lee Contactos y Citas: schema, **todas las pestañas** (`GET /views`, API `2026-03-11`) y filas paginadas. Gzip a R2. No escribe Notion. Campos `files` solo con el nombre. Rotación `R2_RETENCION_DIAS` (default 14). `X-API-Key`. Sin vars R2 → **503** `RESPALDO_NO_CONFIGURADO`. 200 `{ ok, key, bytesGzip, conteos: { contactos, citas, vistasContactos, vistasCitas }, rotacion }`. Snapshot legible del 23-sep: `snapshot-vistas-campos-laura-23sep.md`. |
...
| `consultar_sugeridas_para_asistente` | Lectura + hidratación acotada | Identificador preferido `whatsapp`; si no coincide, `folio` (reservación o boleto) identifica e hidrata la conversación actual en Plática. `sponsorEmpresa` resuelve la empresa aproximada del QR y `sponsor_solicitado` valida boleto, giro y tamaño; salta `Quiere Citas=No`, área y soluciones. Devuelve `fase_evento` (`antes`/`durante`/`despues`) y copys contextuales. `citas_para_ofrecer` trae **todas** las confirmadas con `horario_legible` (día + hora), sin tope 3. Campo **`sugeridas`**: solo `Aprobado`; `sugeridas_para_ofrecer` hasta 4 mezcla canceladas reagendables + Aprobado y, si con eso no llega a 4, se completa con las primeras adicionales (que dejan de salir en `opciones_adicionales`); adicionales = Sugerido y luego giro+tamaño. Una cancelada reagendable trae el nombre de la **persona** del sponsor, no la empresa repetida. Al agotar: “Por ahora ya son todas las disponibles.” `reservar_cita` confirma también sin fila Aprobado. |
| `consultar_disponibilidad_cita` | Lectura | Recibe el `sponsorPageId` y `whatsapp` (o `asistentePageId`). Devuelve como máximo 3 bloques libres en `opciones_para_ofrecer`, **en orden cronológico**. Omite bloques donde el asistente ya tiene cita confirmada y cualquier bloque que ya superó `CITAS_MARGEN_MODIFICACION_MINUTOS` (default 5). En el chat, solo lo que encaja con lo pedido (si dijo “jueves tarde”, no recitar mañana). Si pide una hora concreta, pasar `hora=HH:MM` y `fecha` si acotó el día (`horario_solicitado`); esa hora ocupa la casilla que le toca por día y periodo, así que el segundo día no se cae del lote. Si `hay_mas`, otra llamada con `excluirInicios`. Foto; `reservar_cita` / `modificar_cita` revalidan dentro del mutex. |
...
| `disparar_campanas_aprobadas` | Escritura acotada, masiva | Arranca el disparo y **responde al momento** (`estadoCorrida: en_curso`): Plática corta el tool call ~1 min y el lote (11+ personas) tarda varios minutos. El envío sigue en el proceso; `consultarEstado=true` lee avance/`paraInformar`/errores hasta `terminada`. El webhook HTTP sigue esperando el lote completo. Simulación por default; el agente no habilita envío real. |
...
**Citas conversacionales (Laura, 27-ago; canceladas 7-sep; más opciones 14–15-sep; copy 17-sep):** el asistente agenda, reagenda y cancela hablando, sin botones ni WhatsApp Flow. El primer mensaje sigue siendo como máximo 4 de la oferta `Aprobado`. Si pide más, el backend arma `opciones_adicionales` (sin cambiar el cron de matchmaking ni la campaña). El Agente 2 conserva el avance de la pasada aunque vuelva a consultar o cambie una cita. Al agotar, el contacto oye “Por ahora ya son todas las disponibles.”; `copy_sin_mas_opciones` sigue en el payload como dato interno y **no** se pega. Descriptions MCP (17-sep, Carlos) están pensadas para el Agente 2; el subagente del Agente 1 las comparte y su prompt dice ignorar el tono de WhatsApp. `reservar_cita` ya crea o promueve la fila. Ver [`contrato-citas-conversacionales.md`](contrato-citas-conversacionales.md).
...
- **Campañas de matchmaking**:
  ...
  - El envío real exige cuatro envs: `PLATICA_TEMPLATE_OFERTA_INICIAL_1`…`_4`, configuradas como `agendar_cita_inicial_aprobado_1`…`_4` (aprobadas 9-sep). El backend elige la plantilla según haya 1, 2, 3 o 4 sponsors. `{{1}}` = **primer nombre** capitalizado (`primerNombreParaSaludo`); cada variable siguiente contiene un sponsor en formato `1. Marco Trujillo de la empresa _Platica.mx_, expertos en *Omnichannel · CRM / Automatización*`. Desde 14-sep la empresa va en cursiva y las soluciones en negrita, con la sintaxis **nativa de WhatsApp** (`*negrita*`, `_cursiva_`), no la del editor de Meta: probado en vivo, `**texto**` sale en negrita pero deja un asterisco literal a cada lado. El nombre del representante va sin marcas. Los saltos entre sponsors están en el cuerpo fijo aprobado, nunca dentro del parámetro. El representante sigue siendo nombre + apellido paterno; las soluciones son **todas** las coincidencias de `Solucion` con `Soluciones Buscadas`, sin `Otro`. Si no hay intersección por registro legacy se usan las ofrecidas; sin soluciones queda solo persona/empresa. `payloadPara` elimina saltos, tabs y espacios repetidos de cada variable (las marcas no se tocan). El cuerpo conserva el tope de **1024 caracteres**: primero todas las coincidencias y, si no caben, baja el tope por sponsor de una en una hasta dejar solo nombres; como último recurso suelta al sponsor de menor score y cambia a la plantilla de menor cantidad.
  ...
  - **Recordatorio del evento** (`POST /matchmaking/enviar-recordatorio-evento`): **apagado 22-sep.** No comparte el riesgo de las banderas `CAMPANAS_MATCHMAKING_*`: el service sale antes de evaluar ventana o plantilla. Si Coolify tiene un cron diario, puede borrarse; con o sin cron no manda WhatsApp. La lógica vieja queda en el archivo por si se vuelve a pedir.
