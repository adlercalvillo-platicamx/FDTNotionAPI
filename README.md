...
| `consultar_disponibilidad_cita` | Lectura | Recibe el `sponsorPageId` y `whatsapp` (o `asistentePageId`). Devuelve como máximo 3 bloques libres en `opciones_para_ofrecer`. Omite bloques donde el asistente ya tiene cita confirmada. Sin `fecha`: casillas Día 1 Mañana / Día 1 Tarde / Día 2 (relleno si una casilla está vacía). Con `fecha`, solo ese día. Omite bloques que ya superaron el margen temporal de modificación. Si `hay_mas`, otra llamada con `excluirInicios` (ya implementado). Foto; `reservar_cita` / `modificar_cita` revalidan dentro del mutex (sponsor y asistente). |
