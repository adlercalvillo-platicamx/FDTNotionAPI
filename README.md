...
| `consultar_disponibilidad_cita` | Lectura | Recibe el `sponsorPageId` y devuelve como máximo 3 bloques libres en `opciones_para_ofrecer` (ISO + horario legible). Sin `fecha`, alterna Mañana/Tarde y puede cruzar días; con `fecha`, respeta ese día. Omite bloques que ya superaron el margen temporal de modificación. Si `hay_mas`, otra llamada con `excluirInicios`. Foto; `reservar_cita` / `modificar_cita` revalidan dentro del mutex. |
...
- **Campañas de matchmaking**:
  ...
  - El envío real exige `PLATICA_TEMPLATE_OFERTA_INICIAL` aprobada en Meta. **El contrato final de parámetros sigue pendiente** mientras se diseña/aprueba la plantilla; podría no incluir horarios. El payload actual de simulación conserva provisionalmente nombre, texto con hasta 4 sponsors/soluciones y horario 1–3. Ajustarlo al contrato aprobado antes de habilitar envío real.
  ...
  - Si la plantilla aprobada incluye horarios, salen de **un solo sponsor**: el de mayor score con al menos un bloque libre. Si el top está lleno, se recorre el resto por score. La selección compartida con `consultar_disponibilidad_cita` toma el más próximo y alterna Mañana/Tarde, cruzando días cuando hace falta. `CITAS_CORTE_MANANA_TARDE` (default `14:00`) define el corte. La grilla operativa sigue usando `CITAS_DURACION_BLOQUE_MINUTOS` (hoy 30 min).
