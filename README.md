...
**Tamaño libre + Speaker (1–4 sep):** `Tamaño de Negocio` es `rich_text`; el parser conserva lectura dual de `rich_text`/`select` durante la transición y clasifica por prefijo Grande/Mediana/Pequeña/Micro sin distinguir acentos. Desde 4-sep, un tamaño nuevo entra solo si el sponsor lo eligió en `Etapa Cliente Buscada`; Pequeña/Micro pueden entrar, pero no reciben bono de tamaño. Otros textos son respuestas legacy de Etapa y siguen cayendo al fallback de `Madurez Negocio (Exa)` (Consolidado/PyME), sin cruzarse con la selección del sponsor. `Speaker` y `Presencial VIP` mantienen el bypass de tamaño decidido por Adler; Giro no tiene bypass. **Ranking (3-sep):** `Speaker`/VIP ×1.4, Presencial ×1.15, Virtual ×1.0; oro molido (+1000) no se multiplica. Pendiente de validar con Laura/Liz.
