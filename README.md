...
| `sugerir_matches_para_sponsor` | Escritura acotada | Matchmaking para un sponsor específico. `escribirEnNotion` default `false` (dry-run) — con `true`, crea una fila `Sugerido` en `Citas` por candidato. Capa 1: Giro/Industria (Marca de moda, Retailer, Manufactura; VIP sin excepción), `Quiere Citas 1a1` excluye solo `'No'`, Tamaño de Negocio / Madurez Exa (**Presencial VIP sí salta este filtro**, vía `ticketTipo`, 1-sep). **No** filtra por `Etapa de Negocio` (28-ago). El objeto `sponsor` del return incluye `calendarioGoogleId` desde el 13-ago |
