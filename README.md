...
**Tamaño, Área y Speaker (7-sep):** `Tamaño de Negocio` es `rich_text`; el parser conserva lectura dual de `rich_text`/`select` durante la transición y clasifica por prefijo Grande/Mediana/Pequeña/Micro sin distinguir acentos. Un tamaño declarado entra solo si el sponsor lo eligió en `Etapa Cliente Buscada`; cualquiera de los tamaños elegidos suma **+100**, sin jerarquía entre Grande/Mediana/Pequeña/Micro. Los registros legacy sin tamaño declarado conservan el fallback de `Madurez Negocio (Exa)`: Consolidado +80 y PyME +40. `Speaker` y `Presencial VIP` saltan únicamente este filtro de tamaño y reciben +100 solo cuando su tamaño sí coincide. Área ahora es filtro duro si el sponsor llenó `Puestos Buscados`: coincidencia exacta entra y suma +40; vacía/`Otro` entra como dato desconocido sin puntos; una conocida distinta queda fuera, también para VIP/Speaker. Soluciones: primera coincidencia +30, segunda +10, tercera +10, máximo +50, aunque la explicación conserva la lista completa. Se mantienen `Speaker`/VIP ×1.4, Presencial ×1.15, Virtual ×1.0; oro molido (+1000) no se multiplica. Las `Notas` abren con la vía de entrada de tamaño y ya no incluyen la cuota cambiante.
