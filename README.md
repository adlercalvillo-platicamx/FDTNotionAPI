...
| `consultar_sugeridas_para_asistente` | Lectura | Campo **`sugeridas`**: solo filas `Aprobado`. **`sugeridas_para_ofrecer` hasta 4** mezcla primero canceladas reagendables y luego `Aprobado`. Si pide más: **`opciones_adicionales_para_ofrecer`** (Sugerido, luego sponsors por giro + tamaño; Grande/Consolidado a todos; PyME = Mediana+Pequeña). El agente las recorre por pasadas: la campaña cuenta como Aprobado ya mostrado; consultar otra vez, reservar, modificar o cancelar no reinicia la pasada. Cada lote que deja sponsors sin decir cierra preguntando si quiere elegir o que le busquen más. El último cierra con **`copy_sin_mas_opciones`**, texto aprobado que viaja en el payload precisamente para que el agente lo copie literal en vez de redactarlo (reconstruirlo de memoria lo exponía a sus reglas de tono, que le recortaban una frase). Un pedido posterior inicia otra pasada. `soluciones_en_comun` se presenta como “expertos en” y `otras_soluciones` como “También ofrecen”. `reservar_cita` confirma también sin fila Aprobado. Identificador: `whatsapp`. |
...
**Citas conversacionales (Laura, 27-ago; canceladas 7-sep; más opciones 14–15-sep):** el asistente agenda, reagenda y cancela hablando, sin botones ni WhatsApp Flow. El primer mensaje sigue siendo como máximo 4 de la oferta `Aprobado`. Si pide más, el backend arma `opciones_adicionales` (sin cambiar el cron de matchmaking ni la campaña). El Agente 2 conserva el avance de la pasada aunque vuelva a consultar o cambie una cita; después del aviso de que se agotaron los matches puede empezar otra pasada con los sponsors todavía disponibles. Ese aviso es `copy_sin_mas_opciones`: el texto lo manda el backend en el payload y el agente lo copia literal, porque redactarlo él terminaba en versiones recortadas y ambiguas. `reservar_cita` ya crea o promueve la fila. Ver [`contrato-citas-conversacionales.md`](contrato-citas-conversacionales.md).
