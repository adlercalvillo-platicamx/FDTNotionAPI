...
| GET | `/citas/sugeridas?whatsapp=...` | Solo lectura. Identifica al asistente por teléfono (alias `telefono=`). **`sugeridas`** = solo `Aprobado`. Misma forma que el MCP: `sugeridas_para_ofrecer`, `opciones_adicionales` (Sugerido + giro/tamaño), `citasConfirmadas`. **Ningún cliente HTTP activo en Plática**; el agente usa MCP. Sin match → `404 CONTACTO_NO_RESUELTO`. |
...
| `consultar_sugeridas_para_asistente` | Lectura | Campo **`sugeridas`**: solo filas `Aprobado`. **`sugeridas_para_ofrecer` hasta 4** mezcla primero canceladas reagendables y luego `Aprobado`. Si pide más: **`opciones_adicionales_para_ofrecer`** (Sugerido, luego sponsors por giro + tamaño; Grande/Consolidado a todos; PyME = Mediana+Pequeña). El agente las recorre por pasadas: la campaña cuenta como Aprobado ya mostrado; consultar otra vez, reservar, modificar o cancelar no reinicia la pasada. Lote exploratorio: solo el lote; si hay más, “Todavía hay más.” sin pregunta. Al agotar: **“Por ahora ya son todas las disponibles.”** (`copy_sin_mas_opciones` viaja en el payload como dato interno; no se pega). Si insiste: “No, por ahora no hay otra.” `soluciones_en_comun` / `otras_soluciones` son etiquetas internas, no copy. `reservar_cita` confirma también sin fila Aprobado. Identificador: `whatsapp`. |
