...
La generación periódica de sugerencias es externa al proceso: cron HTTP cada 6h a `POST /matchmaking/sugerir-todos` con `X-API-Key`. Nunca usar ese cron para enviar WhatsApp. La oferta inicial única requiere disparo humano y permanece en simulación hasta el opt-in de envío real. Plantilla aprobada en Meta (28-ago): `agendar_cita_inicial` → env `PLATICA_TEMPLATE_OFERTA_INICIAL`. **Contrato: 2 variables** — `{{1}}` **solo el primer nombre**, capitalizado (Ticketópolis escribe "ANA MARIA PEREZ" en `Nombre`; `primerNombreParaSaludo` recorta al primer token y baja las mayúsculas), `{{2}}` hasta 4 sponsors en **un solo renglón**, numerados, con barra (`1) *Marco Trujillo* de *Platica.mx* (solución) | 2) …`). El representante es nombre + apellido paterno (`nombreRepresentanteParaOferta`: 2 tokens se quedan; 3+ usa el primero y el penúltimo, p. ej. Zuleyma Jessamine Chávez Coronado → Zuleyma Chávez). Negrita en persona y empresa. El `\r` pasa el filtro de Meta pero es retorno de carro y se come los sponsors siguientes — no usarlo (probado 2-sep). En `{{2}}` va la **intersección** de `Solucion` del sponsor con `Soluciones Buscadas` del asistente, **máximo 2 por sponsor** y sin el comodín `Otro`; sin intersección (registro legacy) cae a lo que ofrece el sponsor, y si no queda nada va `*Nombre* de *Empresa*` o solo `*Empresa*` si falta el representante. No volcar el catálogo completo: el cuerpo armado no puede pasar **1024 caracteres** o Meta rechaza el envío (caso real 28-ago: 4 sponsors × 5 soluciones = 1035). `textoSugerencias` calcula el techo de `{{2}}` como 1024 − cuerpo fijo − `{{1}}` − colchón y recorta en este orden: 2 soluciones → 1 → solo nombres → soltar el último sponsor. Los valores de variable tampoco pueden traer saltos de línea, tabs ni más de 4 espacios seguidos, por eso `payloadPara` los sanea. **Sin horarios**: los ofrece el agente en la conversación, y ya no se salta a quien no tenga un bloque libre en ese instante (`SIN_HORARIOS_SUGERIDOS` se retiró). La oferta solo se envía a quien no tenga `Última Campaña Enviada`; B/C1/C2 quedan fuera y la única reactivación automática activa es el follow-up 72h. El recordatorio-reactivación del evento es `POST /matchmaking/enviar-recordatorio-evento` (cron diario en Coolify; el endpoint no hace nada hasta 14 días antes del primer día de `CITAS_FECHAS_EVENTO`). Limpiar la cola acumulada antes del primer envío real es `scripts/one-shots/marcar-cola-sin-enviar.js` (`soloMarcar`); pide `--confirmar` y después escribir el título real de Citas. No hay endpoint ni tool MCP para eso.

El follow-up amistoso usa la plantilla aprobada `followup_72hrs` con `{{1}} = primerNombreParaSaludo`. Plática `message.created` es la verdad de respuesta: solo `direction=incoming` posterior a `Fecha Última Campaña`; `canSendDirectMessage`, delivery/read y `lastUpdate` **no** prueban respuesta. El webhook firmado marca `Respondió Oferta Inicial`/fecha en Contactos; el cron cada 15 min reconsulta el historial por teléfono/canal antes de enviar, omite a quien ya tenga cita y solo opera lun–vie 09:00–18:00 CDMX después de 72 horas naturales. Banderas separadas y seguras: `FOLLOWUP_72H_MODO_SIMULACION=true`, `FOLLOWUP_72H_ENVIO_REAL_HABILITADO=false`. Estado idempotente en Contactos: `Estado Follow-up 72h` + `Fecha Follow-up 72h`; `Reactivaciones Enviadas` incrementa al éxito. Toda plantilla (`POST /v1/messages/template` y texto de sesión) lleva `responderAgentId` = `PLATICA_RESPONDER_AGENT_ID` (`c1IYnFsr0Jzfqq4NeLAs` en FDT). Canal confirmado Adler 4-sep: `wb-1167456423128610`. No usar el de Clarita ni el de Marketing.
