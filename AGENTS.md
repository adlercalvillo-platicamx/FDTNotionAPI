...
Los custom fields operativos de Plática son manuales (`automatic=false`) y se hidratan desde Notion. `Citas confirmadas` es `textList` con empresa + fecha/hora y `Número de citas confirmadas` excluye `Sugerido`, `Aprobado` y `Cancelada`. En Notion, el conteo correcto del asistente es `Citas Confirmadas Asistente (Count)` sobre `Citas como asistente`; el campo histórico `Citas Confirmadas (Count)` cuenta al contacto como sponsor. `Quiere Citas 1a1` no se copia a Plática: el filtro queda en matchmaking.
