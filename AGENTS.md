...
| Sugeridas del asistente | GET `/citas/sugeridas?whatsapp=` (mismo payload que el MCP: `sugeridas` = Aprobado; `opciones_adicionales` = Sugerido + giro/tamaño). | `consultar_sugeridas_para_asistente` (`whatsapp`; capa 1 = cancelada reagendable + Aprobado, tope 4; capa 2 = `opciones_adicionales`; + confirmadas/canceladas). En chat se recorren por pasadas; campaña/consulta/reserva/cambio/cancelación no reinician la pasada. Al agotar: “Por ahora ya son todas las disponibles.” `copy_sin_mas_opciones` viaja en el payload como dato interno; no se pega. |
