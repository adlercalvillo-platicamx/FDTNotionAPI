...
- **Tamaño de Negocio** es `rich_text` desde 1-sep; el parser tolera `select` durante la transición y clasifica por prefijo sin acentos. Si trae el formato nuevo (Grande/Mediana/Pequeña/Micro), entra solo cuando esa categoría está en `Etapa Cliente Buscada` del sponsor (desde 2-sep ese nombre técnico significa **tamaños buscados**). Si no trae tamaño nuevo, conserva el fallback legacy Exa Consolidado/PyME, independiente de la selección del sponsor. **Presencial VIP y Speaker saltan este filtro** vía `ticketTipo`; el de Giro no.
