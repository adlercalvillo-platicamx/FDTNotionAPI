...
- **Tamaño de Negocio** es `rich_text` desde 1-sep; el parser tolera `select` durante la transición y clasifica por prefijo sin acentos. Si trae el formato nuevo (Grande/Mediana/Pequeña/Micro), entra solo cuando esa categoría está en `Etapa Cliente Buscada` del sponsor (desde 2-sep ese nombre técnico significa **tamaños buscados**). Cualquier tamaño solicitado suma +100, sin jerarquía entre tamaños; si hay tamaño declarado, no se acumula Exa. Si no trae tamaño nuevo, el fallback Exa suma Consolidado +80 / PyME +40, independiente de la selección del sponsor. **Presencial VIP y Speaker saltan este filtro** vía `ticketTipo`, pero reciben +100 solo si su tamaño sí coincide; Giro y Área no tienen bypass.
- **Speaker** es `Categoria=Asistente`, entra al pool. Filtro de Giro igual que el resto; salta el de tamaño. En ranking, Speaker y Presencial VIP usan el mismo multiplicador (`×1.4`); Presencial `×1.15`; Virtual `×1.0`. No hay bono fijo de 500/150. Oro molido (+1000) no se multiplica. Decisión Adler 3-sep, no confirmada por Laura/Liz.
- **Soluciones** tienen rendimiento decreciente: primera coincidencia +30, segunda +10, tercera +10, tope +50. Se conserva la lista completa para explicar el match. Las `Notas` empiezan por la vía de entrada de tamaño (declarado, fallback Exa o bypass VIP/Speaker) y no incluyen `cuota_pendiente`; la cuota solo define `topN = Citas Minimas Prometidas + MARGEN_CANDIDATOS`.
