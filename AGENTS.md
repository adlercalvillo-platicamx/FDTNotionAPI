...
- Correos: dos envíos distintos (sponsor con datos del asistente y mesa; asistente corto, **sin** contacto del sponsor). Para `Ticket / Tipo Asistencia = Virtual`, confirmar y modificar omiten mesa, sede e indicación de llegada en el cuerpo/`DESCRIPTION`; explican que el Meet llega ~15 min antes por WhatsApp + invitación de Google. La mesa sigue en Notion y en el correo del sponsor. Presencial/VIP/Speaker conservan el copy físico. El WhatsApp de soporte va **solo** en el correo del asistente. 3 reintentos SMTP inmediatos por envío. `emailsExtra` / `asistentes_email` van al correo del asistente.
