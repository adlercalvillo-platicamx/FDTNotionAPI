...
- **Tamaño de Negocio** es `rich_text` desde 1-sep; el parser tolera `select` durante la transición y clasifica por prefijo sin acentos. Si trae el formato nuevo (Grande/Mediana/Pequeña/Micro), entra solo cuando esa categoría está en `Etapa Cliente Buscada` del sponsor (desde 2-sep ese nombre técnico significa **tamaños buscados**). El filtro de entrada no ordena; el ranking sí: Grande +100, Mediana +70, Pequeña +58, Micro +30. Si hay tamaño declarado, no se acumula Exa. Si no trae tamaño nuevo, el fallback Exa suma Consolidado +80 / PyME +40, independiente de la selección del sponsor. **Presencial VIP y Speaker saltan este filtro** vía `ticketTipo`, pero reciben esos puntos solo si su tamaño sí coincide; Giro, Área y Soluciones no tienen bypass de boleto.
